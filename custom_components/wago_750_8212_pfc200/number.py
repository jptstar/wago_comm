...
async def async_setup_entry(hass: HomeAssistant, entry: ConfigEntry, async_add_entities: AddConfigEntryEntitiesCallback) -> None:
    ...
    points = [p for p in runtime.points if p.get("enabled", True) and p.get("platform") == PLATFORM_NUMBER]
...
class WagoNumber(WagoPointEntity, NumberEntity):
    def __init__(self, coordinator, entry, point) -> None:
        ...
        self._attr_native_max_value = float(point.get("max", 65535.0 * float(point.get("scale", 1.0) or 1.0)))
